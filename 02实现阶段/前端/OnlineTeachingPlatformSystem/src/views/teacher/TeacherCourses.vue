...
import { fetchCourseClassifications, fetchTeacherCourses } from '@/api/data'; // 假设您已经封装了API
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchCourses = async () => {
      try {
        const response = await fetchTeacherCourses(accountId);
      ...
      <el-card class="box-card">
        <template #header>
          <div class="course-header">
            <h2>本人全部课程</h2>
            <el-button type="primary" @click="$router.push('/profile/teacher/create')">创建课程</el-button>
          </div>
        </template>
        <div v-if="courses.length > 0">
          <el-row :gutter="20">
            <el-col
              ...
              :key="course.courseId"
            >
              <el-card class="course-card" body-style="padding: 0;" @click="$router.push(`/course/teacher/${course.courseId}/info`)">
                <el-row>
